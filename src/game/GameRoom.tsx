...
const findUnusedColor = (takenColors: Set<PlayerColor>): PlayerColor | undefined => {
...
function ColorPicker({
    ...
    const setColor = useCallback((requestedColor?: PlayerColor) => {
        const playerColorsRef = ref(db, `rooms/${roomId}/state/playerColors`);
        runTransaction(playerColorsRef, (currentColors: Record<string, PlayerColor> | null) => {
            ...
            {PLAYER_COLORS.map(c =>
                <li key={c}>
                    <button
                        ...
                        disabled={selectedColor !== c && Object.values(playerColors).includes(c)}
                        onClick={() => setColor(c)}
                    >
                        {selectedColor === c && (
                            <svg className="ColorPicker-icon" xmlns="https://www.w3.org/2000/svg" version="1.1" viewBox="0 0 17.837 17.837">
